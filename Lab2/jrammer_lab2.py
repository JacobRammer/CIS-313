"""
Jacob Rammer
Lab 2
2/4/2020
"""


class PriorityQueue:
    """
    Class for a priority queue using binary heaps
    """

    def __init__(self, maxSize):
        """
        init method for pq
        """

        self._maxSize = maxSize
        self._length = 0
        self._heap = [None] * (maxSize + 1)  # since idx starts @ 1

    def isFull(self):
        """
        Checks to see if the heap is full
        for successful push onto heap
        :return: true if full, else false
        """

        return self._length == self._maxSize

    def swap(self, index, ticket):
        """
        Swap the child and parent node if required to keep proper
        tree properties
        :param index: index of the node
        :param ticket: mealticket
        :return: void
        """

        self._heap[index] = ticket
        while index > 1 and \
                self._heap[self.getParent(index)] < self._heap[index]:
            self._heap[index], self._heap[self.getParent(index)] = \
                self._heap[self.getParent(index)], self._heap[index]
            index = self.getParent(index)

    def insert(self, ticket):
        """
        Insert a mealticket into the heap while keeping
        parent child structure
        :param key: mealticket
        :return: void
        """

        if self.isFull():
            return False
        self._length += 1
        self._heap[self._length] = -1  # a mealticket's total will never be < 0
        self.swap(self._length, ticket)

    def getParent(self, index):
        """
        Return the index of the parent node
        :param index: int
        :return: int
        """

        return index // 2

def main():
    pq = PriorityQueue(7)
    pq.insert(5)
    pq.insert(4)
    pq.insert(6)
    pq.insert(9)
    print(pq._length)
    pq.insert(8)
    print(pq._heap)

main()
